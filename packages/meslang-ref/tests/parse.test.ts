...
test("examples/manga/station-two-pages.mes: == pages and % restart", () => {
  ...
  const p2last = page2[page2.length - 1]!;
  assert.equal(p2last.dialogue.trim(), "");
  assert.ok(p2last.decorators.some((d) => d.kind === "frame" && d.value === "4"));
  assert.ok(p2last.decorators.some((d) => d.kind === "beat"));
  const withAttrs = page1
    .map((p) => firstCharacter(p))
    .filter((ch): ch is NonNullable<typeof ch> => ch != null && Object.keys(ch.attrs).length > 0);
  assert.ok(withAttrs.some((ch) => ch.attrs["表情"] === "焦り" && ch.attrs["姿勢"] === "前かがみ"));
  assert.ok(withAttrs.some((ch) => ch.attrs["吹き出し"] === "心の声"));
});
