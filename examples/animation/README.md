# アニメ字コンテの見本

字コンテ用途の **原稿（字コンテ下地）** の短い見本です。  
きれいなカット表や絵コンテそのものではなく、カット番号・カメラ・尺・セリフをさっと残すためのテキストです。  
呼び分けは [用語集（原稿と二次出力）](../../docs/spec/05-glossary.md#原稿と二次出力まぎらわしいことば) / [用語集（コマとカット）](../../docs/spec/05-glossary.md#コマとカットまぎらわしいことば) をどうぞ。

## ファイル

| ファイル | 内容 |
|----------|------|
| [station-conte.mes](station-conte.mes) | 駅前の再会を5カット。カメラ・尺・音に加え、ツール用の `?` メモも混ぜています |

## 読み方のヒント

- `%CUT-…` … カットの区切り（漫画のコマと同じ「画面の区切り」。アニメでは **カット** と呼ぶ）
- `^` … カメラの動きやレンズ感
- `&` … 秒数の目安
- `#` / `$` … 演技・状況と、聞こえる音
- `?` … 制作ツール向けの逃げ道（レイアウト案や背景 ID など）。仕様は意味を固定しません → [デコレーター仕様の「`?` について」](../../docs/spec/02-decorators.md)

くわしい記号の見比べは [メディアプロファイル](../../docs/spec/03-media-profiles.md) をどうぞ。  
カット表の仮形（二次出力）は [絵コンテ／カット表](../../docs/spec/07-conte-table.md) と、参照パーサの `npm run conte` をどうぞ。  
AI に字コンテ起こしやカット表整理を頼むときのひな形は [AI 向けガイド](../../docs/spec/04-ai-reading.md#アニメ字コンテ起こし) にあります。

## カット表 JSON の目視（任意）

見本を二次出力の形で一度眺めるとき:

```bash
cd packages/meslang-ref
npm run conte -- ../../examples/animation/station-conte.mes
# 形チェック込み（Medo → カット表）
node --experimental-strip-types src/cli.ts --conte --validate ../../examples/animation/station-conte.mes
```

出てきた JSON で、だいたい次を確認します。

- `version` が `conte-table/0.0`（`--validate` でも同じ形を見ます）
- カットが5行（`CUT-001` … `CUT-005`）
- 各行にカメラ（`^`）・尺（`&`）が入っている
- `?bg` や `?layout` は `ext` 側に残り、カメラ欄へ混ざっていない

機械形の本命はツールです。AI には人向けの表だけ頼む、という分担は [AI 向けガイド](../../docs/spec/04-ai-reading.md#カット表への整理二次出力の補助) と同じです。
