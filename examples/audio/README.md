...
三場面目「ランチへ」では、セリフのないピース（`$` / `!` / `&` / `#` だけ）で信号待ちの「間」を置いてから、また会話に戻ります。  
漫画の無言コマと同じく、音だけの拍も原稿の一部です。同じ信号待ちの拍を漫画側で置いた例は [station-two-pages.mes](../manga/station-two-pages.mes) の2ページ目 `%4` です。
### セリフ文字数を数える（任意）
